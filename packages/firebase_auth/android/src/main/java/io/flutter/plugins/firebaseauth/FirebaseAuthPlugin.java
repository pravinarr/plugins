...
public class FirebaseAuthPlugin implements MethodCallHandler {
    ...
    public void onMethodCall(MethodCall call, Result result) {
        switch (call.method) {
            ...
            case "verifyPhoneNumber":
                ...
                break;
            case "mfaVerifyPhoneNumber":
                handleMfaVerifyPhoneNumber(call, result, getAuth(call));
                break;
            case "mfaVerifyPhoneNumberToEnroll":
                handleMfaVerifyPhoneNumberToEnroll(call, result, getAuth(call));
                break;
            case "verifyCodeAndEnrollMfa":
                handleVerifyCodeAndEnrollMFA(call, result, getAuth(call));
                break;
            case "unenrollFromMfa":
                handleUnenrollFromMfa(call, result, getAuth(call));
                break;
            case "verifyCodeAndSignIn":
                handleVerifyCodeAndSignIn(call, result, getAuth(call));
                break;
    ...
    private void handleUnenrollFromMfa(MethodCall call, final Result result, FirebaseAuth firebaseAuth){
        List<MultiFactorInfo> options = firebaseAuth.getCurrentUser().getMultiFactor().getEnrolledFactors();
        firebaseAuth.getCurrentUser().getMultiFactor()
                .unenroll(options.get(0))
                .addOnCompleteListener(new TaskVoidCompleteListener(result));
    }

    private void handleVerifyCodeAndSignIn(MethodCall call, final Result result, FirebaseAuth firebaseAuth) {
    ...
    private void handleVerifyCodeAndEnrollMFA(MethodCall call, final Result result, FirebaseAuth firebaseAuth) {
        Map<String, String> arguments = call.arguments();
        String verificationId = arguments.get("verificationId");
        String smsCode = arguments.get("smsCode");
    ...
    private void handleMfaVerifyPhoneNumberToEnroll(
        ...
        firebaseAuth.getCurrentUser().getMultiFactor().getSession()
                .addOnCompleteListener(
                                ...
                                if (task.isSuccessful()) {
                                    ...
                                    PhoneAuthOptions phoneAuthOptions =
                                            PhoneAuthOptions.newBuilder()
                                                    ...
                                                    .setCallbacks( new CustomPhoneVerificationCallback(handle))
                                                    .build();
    ...
    private void handleMfaVerifyPhoneNumber(
            MethodCall call, Result result, final FirebaseAuth firebaseAuth) {
        Map<String, Object> arguments = call.arguments();
        final int handle = (int) arguments.get("handle");

        PhoneMultiFactorInfo selectedHint =
                (PhoneMultiFactorInfo) multiFactorResolver.getHints().get(0);
        ...
        if (call.argument("forceResendingToken") != null) {
            ...
            PhoneAuthProvider.getInstance()
                    .verifyPhoneNumber(
                            ...
                            registrar.activity(),
                            new CustomPhoneVerificationCallback(handle),
                            forceResendingToken);
        } else {
            PhoneAuthProvider.getInstance()
                    .verifyPhoneNumber(
    ...
    private class CustomPhoneVerificationCallback extends PhoneAuthProvider.OnVerificationStateChangedCallbacks {
        private final int handle;

        CustomPhoneVerificationCallback(int handle){
            this.handle = handle;
        }
